...
import CUET_MOBILEVIEW from "./CUET_MOBILEVIEW";
...
export default function CUPT() {
    ...
    // Dynamic subjects based on examInfo.subjectName
    const dynamicExamRows = useMemo(() => {
        const subjects = new Set<string>();
        planAndPricing.forEach((plan: any) => {
            plan.exams?.forEach((e: any) => {
                if (e.examInfo?.subjectName) subjects.add(e.examInfo.subjectName);
            });
        });
        return Array.from(subjects).map(sub => ({ label: sub, key: sub }));
    }, [planAndPricing]);
    ...
    const getReferralDiscount = () => {
        ...
        if (ref.isActive && new Date(ref.expiry) > new Date() && ref.usedCount < ref.maxUsage) {
                                    ...
                                    {planAndPricing.map((plan: any) => {
                                        ...
                                        return (
                                            <th
                                                ...
                                                className={`align-bottom rounded-t-lg pb-1 transition-all border-t-2 border-x-2 ${hoveredPlan === plan._id ? 'border-[#C8DCFE]' : 'border-transparent'} ${plan.title.includes('Pro') || plan.title.includes('CUET') ? 'bg-[#F4F7FA]' : ''}`}
                                            >
                                                <div className="text-center">
                                                  <h3 className="text-xl font-medium text-[#FF5635]  uppercase my-1">{plan.title.split(' ')[0]}</h3>
                                                    ...
                                                    <p className="text-3xl font-medium text-black">₹ {Math.round(finalPrice)}</p>
                                                    {referralAmount > 0 && (
                                                        <p className="text-xs text-green-600 font-medium">{referralDiscount}% Referral Applied</p>
                                                    )}
                                                </div>
                                            </th>
                                        );
                            ...
                            <tbody>
                                <tr className="text-left">
                                    ...
                                    {planAndPricing.map((p: any) => (
                                        <td key={p._id} onMouseEnter={() => setHoveredPlan(p._id)} onMouseLeave={() => setHoveredPlan(null)} className={`${hoveredPlan === p._id ? 'border-x-2 border-[#C8DCFE]' : ''} ${p.title.includes('Pro') || p.title.includes('CUET') ? 'bg-[#F4F7FA]' : ''}`}></td>
                                    ))}
                                </tr>
                                ...
                                {dynamicExamRows.map((row) => (
                                    <tr key={row.key}>
                                        ...
                                        {planAndPricing.map((plan: any) => {
                                            const examData = plan.exams?.find((e: any) => e.examInfo?.subjectName === row.key);
                                            return (
                                                <td key={plan._id} onMouseEnter={() => setHoveredPlan(plan._id)} onMouseLeave={() => setHoveredPlan(null)} className={`border-t border-[#C8DCFE] text-center ${hoveredPlan === plan._id ? 'border-[#C8DCFE] border-x-2' : 'border-t border-x-0'} ${plan.title.includes('Pro') || plan.title.includes('CUET') ? 'bg-[#F4F7FA]' : ''}`}>
                                                    <span className="font-medium text-[#ff5635]">{examData ? examData.mockCount : "—"}</span>
                                                </td>
                                            );
                                        })}
                                    </tr>
                                ...
                                <tr>
                                    <td></td>
                                    {planAndPricing.map((plan: any) => (
                                        <td key={plan._id} onMouseEnter={() => setHoveredPlan(plan._id)} onMouseLeave={() => setHoveredPlan(null)} className={`p-4 border-x-2 ${hoveredPlan === plan._id ? 'border-[#C8DCFE]' : 'border-transparent'} ${plan.title.includes('Pro') || plan.title.includes('CUET') ? 'bg-[#F4F7FA]' : ''}`}>
                                            <div className="flex flex-col items-center gap-1">
                                                <button
                                                    onClick={() => handlePayment(plan)}
                                                    className={`py-3 px-6 cursor-pointer rounded-xl text-sm font-medium transition-all w-full ${plan.title.includes('Elite') ? 'bg-[#FFBD00]' : (plan.title.includes('Pro') || plan.title.includes('CUET')) ? 'bg-[#FF5635] text-white' : 'bg-[#EBE9FF]'}`}
                                                >
                                                ...
                                                <div className="w-full mt-2">
                                                    ...
                                                    <div className="flex h-8 w-full border border-gray-200 rounded-md overflow-hidden bg-white">
                                                        ...
                                                        <button
                                                            ...
                                                            className={`text-[10px] cursor-pointer px-2 font-medium border-l ${plan.title.includes('Elite') ? 'bg-[#FFBD00]' : (plan.title.includes('Pro') || plan.title.includes('CUET')) ? 'bg-[#FF5635] text-white' : 'bg-[#EBE9FF]'}`}
                                                        >
                                                            {isVerifying === plan._id ? <Loader2 className="animate-spin w-3 h-3" /> : "APPLY"}
                                                        </button>
                                                    </div>
                                                    {couponStatuses[plan._id] && (
                                                        <p className={`text-[12px] font-normal mt-1 text-center ${couponStatuses[plan._id].type === 'success' ? 'text-green-600' : 'text-red-500'}`}>
                                ...
                                <tr>
                                    <td></td>
                                    {planAndPricing.map((plan: any) => (
                                        <td key={plan._id} onMouseEnter={() => setHoveredPlan(plan._id)} onMouseLeave={() => setHoveredPlan(null)} className={`h-4 rounded-b-xl border-x-2 border-b-2 ${hoveredPlan === plan._id ? 'border-[#C8DCFE]' : 'border-transparent'} ${plan.title.includes('Pro') || plan.title.includes('CUET') ? 'bg-[#F4F7FA]' : ''}`}></td>
                                    ))}
                                </tr>
                            </tbody>
                        </table>
