...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (singleQuestion && singleQuestion.length==0 && !singleQuestion?.[0]) return;
 const q = singleQuestion?.[0] ?? null;
  ...
  const sectionQuestions = useMemo(() => {
    if (!data?.details || !selectedSection) return [];
    return data.details
      .filter((q: any) => {
        const qSecId = typeof q.section === "object" ? q.section._id : q.section;
        return qSecId === selectedSection.sectionId;
      })
