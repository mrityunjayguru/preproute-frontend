  ...
  return (
    <div className="col-span-12 lg:col-span-7 h-fit bg-white">
      ...
      <div className="flex justify-between items-center mb-6 px-5 font-poppins">
          ...
          {subExamOptions.length > 0 && (
            <div className="w-48">
              <Select
                ...
                value={
                  formData.subExamTypeId
                    ? subExamOptions.find(
                        (opt: any) => opt.value === formData.subExamTypeId,
                      )
                    : null
                }
            ...
            <div className="w-48">
              <Select
                ...
                value={
                  formData.examId
                    ? examOptions.find(
                        (opt: any) => opt.value === formData.examId,
                      )
                    : null
                }
        ...
        <h2
          onClick={viewallexam}
          className="text-[#FF5635] text-sm font-normal cursor-pointer hover:underline"
        >
          View All
        </h2>
      </div>
      ...
      <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar px-5 pb-5">
        {exams &&
          exams.length > 0 &&
          exams.map((exam: any, i: number) => <ExamCard key={i} exam={exam} />)}
      </div>
    </div>
  );
};
    ...
    <>
      ...
      <div className="flex flex-col w-full rounded-[8px] bg-gradient-to-t from-[#F0F9FF] to-white border border-[#E6F4FF] px-5 py-5 ">
        ...
        <div className="flex justify-between items-start mb-1">
          ...
          {exam?.sectionId ? (
            <>
              <h1>{exam?.sectionCompletion[0]?.sectionName}</h1>
            </>
          ) : null}
          <span
            className={`text-lg font-dm-sans font-normal ${isDraft ? "text-gray-400" : "text-gray-500"}`}
          >
        ...
        <div className="mb-4">
          <p className="text-sm font-normal  text-gray-800 font-dm-sans">
            ...
            <span className="text-[#FF5635]">
              {exam?.userDetail?.username || "Admin"}
            </span>
          </p>
        </div>
