...
import { getgroup, handlesetUpdategroup } from "@/api/group";

function GroupTable() {
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const data = useSelector((state: any) => state?.group?.group || []);
  console.log(data,"datadata")
  const [search, setSearch] = useState<string>("");
  const fetchUsers = async () => {
    const payload: any = {};
    await dispatch(getgroup(payload));
  };
  ...
  const handleEdit = async (val: any) => {
    ...
    await dispatch(handlesetUpdategroup(val));
    // router.push("users/create");
